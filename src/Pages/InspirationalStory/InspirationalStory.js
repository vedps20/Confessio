...
export default class InspirationalStory extends React.Component {
  ...
  componentDidMount(){
    ...
    firebase.firestore().collection("Inspirational Stories").get().then(snap=>{
    ...
    collection("Inspirational Stories").limit(((this.state.limit<=this.state.max)?this.state.limit:this.state.max)).
    orderBy("Date", "desc").
  ...
  render(){
    ...
    loadMore=()=>{
      ...
      this.setState({limit:(newLimit<this.state.max?newLimit:this.state.max)},()=>{
        firebase.firestore().
          collection("Inspirational Stories").limit(this.state.limit).
          orderBy("Date", "desc").
          onSnapshot(querySnapshot=>{
            const storyTemp = [];
