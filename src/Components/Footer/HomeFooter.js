  ...
  return(
      <View style={style.footerComponent}>
        ...
        <FlatList
          ...
          renderItem={(item)=>{
            return(
            <FooterTag data={item} navigator={props.navigator} Title={props.navigationTitle} tagData={props.dataTag}/>);
          }}
...
function FooterTag(props){ //only for top stories and confessions
  ...
  let details = props.data.item;

  return(
    <Button
    style={style.footerTagButton}
    onPress={()=>{props.navigator.navigate(props.Title,{details})}}
    >
      <Text
      numberOfLines={1}
      ellipsizeMode='tail'
...
function FooterTags(props){  //only for top tags
  ...
  return(
    <Button
    style={style.footerTagButton}
    onPress={()=>{props.navigator.navigate(props.Title,{name})}}
    >
      <Text
      style={{
        textAlign:'center',
