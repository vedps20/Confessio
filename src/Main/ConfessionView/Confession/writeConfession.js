...
export default class WriteAConfession extends React.Component {
    ...
    constructor(props){
        ...
        setTimeout(() => {a.pop();this.setState({attributes:a})}, 5000)
  ...
  onSubmit(){
    ...
    if(title.trim()==""||youAre.trim()==""||description.trim()==""||writingTo.length==0){
      this.setState({err : error});
        ...
        const attr = this.state.writingTo;
        const tagsRef = firebase.firestore().collection('Tags')
        //testing purpose :)        
        // for(let i=0;i<attr.length;i++)
        // { 
        //     tagsRef.doc(attr[i]).set({target: 0,
        //         name: attr[i]
        //     })
        ...
        for(let i=0;i<attr.length;i++)
        { 
            tagsRef.doc(attr[i]).get().then((doc)=>{
                tagsRef.doc(attr[i]).update({
                    target: doc.data().target + 1
                })
            })
